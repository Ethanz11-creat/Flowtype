...
### 音柱重做（频率驱动，本轮做完）
- [ ] **随内容变化**：说低音/「sss」/正常说话，柱子跳出不同图案（不再全体一起涨落）
- [ ] **镜像声波**：~28 根细柱从中线上下对称生长、中间高两边低；紫蓝渐变
- [ ] **静默回落**：录音中不说话时柱子落到低静息线（不僵直、不抖）
- [ ] **手感调参**（如需）：`AudioVisualizer`/`SpectrumAnalyzer` 里 minHz/maxHz、attack/decay、柱数、`.frame(maxHeight:.infinity)` 镜像写法 等真机看着调（spec §9）
